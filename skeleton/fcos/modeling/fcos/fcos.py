...
class FCOS(nn.Module):
    ...
    def predict_proposals_single_image(
        ...
        for (cls_score, bbox_pred, centerness, points) in zip(
            ...
            pre_nms_top_k = candidate_inds.view(-1).sum()
            ...
            per_class = candidate_nonzeros[:, 1]
        

            bbox_pred = bbox_pred[box_loc]
            locations = points[box_loc]

            if candidate_inds.sum().item() > pre_nms_top_k.item():
                scores, topk_ind = scores.topk(pre_nms_top_k, sorted=False)
                per_class = per_class[topk_ind]
                locations = locations[topk_ind]
                bbox_pred = bbox_pred[topk_ind]

            detections = torch.stack([locations[:, 0] - bbox_pred[:, 0], locations[:, 1] - bbox_pred[:, 1], locations[:, 0] + bbox_pred[:, 2], locations[:, 1] + bbox_pred[:, 3]], dim=1)
            
            bboxes = Instances(image_size)
            bboxes.pred_boxes = Boxes(detections)
            bboxes.scores = torch.sqrt(scores)
            bboxes.pred_classes = per_class
            bboxes.locations = locations
